...
namespace NotificationWindow
{
    ...
    public class PopupNotifier : Component
    {
		...
		public event EventHandler HideNotif;
		public string Link;


        private bool disposed = false;
        private PopupNotifierForm frmPopup;
		public PopupNotifierForm form
		{ 
		...
		public int iPopUpIndex { get; set; }
		public int iPopUpLayer { get; set; }
		...
		[Category("Image")]
		[Description("Show the image border")]
        ...
        public PopupNotifier()
        {
			...
			ImageBorderColor = Color.FromArgb(134, 169, 81);
        ...
        public void Popup()
        {
            if (!disposed)
            {
                if (!frmPopup.Visible)
                {
					...
					iOffset = (iPopUpIndex * frmPopup.Size.Height) + (iPopUpIndex * 5);
					
					if (Scroll)
					{
						posStart = Screen.PrimaryScreen.WorkingArea.Bottom - iOffset;
						posStop = Screen.PrimaryScreen.WorkingArea.Bottom - frmPopup.Height - iOffset;
					}
					else
					{
						posStart = Screen.PrimaryScreen.WorkingArea.Bottom - frmPopup.Height - iOffset;
						posStop = Screen.PrimaryScreen.WorkingArea.Bottom - frmPopup.Height - iOffset;
					}

					//int tmp  = Screen.PrimaryScreen.WorkingArea.Bottom - frmPopup.Height - iOffset;
                    ...
                    tmrWait.Interval = Delay + (1000 * (iPopUpIndex-1));
		...
		private void frmPopup_HideNotif(object sender, EventArgs e)
		{
            HideNotif?.Invoke(this, EventArgs.Empty);
        }
