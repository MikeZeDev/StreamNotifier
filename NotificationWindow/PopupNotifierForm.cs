...
namespace NotificationWindow
{
    ...
    public class PopupNotifierForm : System.Windows.Forms.Form
    {
        ...
        private Brush brushImageBorder;
        ...
        private void AllocateGDIObjects()
        {
            ...
            brushImageBorder = new SolidBrush(Parent.ImageBorderColor);
        ...
        private void PopupNotifierForm_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (Parent.Image != null)
            {
                ...
                //Parent.Image = (Image)Helper.ScaleImage(Parent.Image, 80, 80);

                if (Parent.ShowImageborder)
                {
                    try
                    {
                        Bitmap tmp = Parent.Image as Bitmap;
                        using (Graphics g = Graphics.FromImage(tmp))
                        {
                            g.DrawRectangle(new Pen(brushImageBorder, 5), new Rectangle(0, 0, tmp.Width, tmp.Height));
                        }
                        e.Graphics.DrawImage(tmp, Parent.ImagePadding.Left, Parent.HeaderHeight + Parent.ImagePadding.Top, Parent.ImageSize.Width, Parent.ImageSize.Height);
                    }
                    catch (Exception)
                    {
